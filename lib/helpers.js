...
const getElapsedTime = (start, finished = moment()) => moment(finished).diff(moment(start), 'minutes') / 60
...
module.exports.getArchivedPoms = db => db.get('days').value().reduce((acc, day) => {
  day.poms.forEach((x) => {
    acc.push(x)
  })
...
module.exports.addPom = ({ title, estimate = 0 }, db) => {
  const id = uuidv1()
  db
    .set(`poms[${id}]`, {
...
module.exports.inspectPom = ({ id }, db) => {
  let pom = db.get(`poms[${id}]`).value()

  if (!pom) {
    pom = db
      .get('days')
      .reduce((acc, x) => {
        const result = x.poms.find(y => y.id === id)
        if (result) {
          acc = result
        }
